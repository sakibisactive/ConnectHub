...
const SingleToastItem = ({ toast }) => {
  const dispatch = useDispatch();

  useEffect(() => {
    // Automatically close toast notification after 5 seconds (5000ms)
  ...
  return (
    <div className="pointer-events-auto p-4 rounded-2xl bg-slate-900 border border-slate-700/80 shadow-2xl flex items-start justify-between gap-3 animate-slide-up">
...
export const NotificationToast = () => {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50 space-y-2 max-w-sm w-full pointer-events-none">
      {toasts.map((toast) => (
        <SingleToastItem key={toast.id} toast={toast} />
      ))}
    </div>
  );
};
